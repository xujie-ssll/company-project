        ...
        <div class="drawer-content">
          <div class="form-section">
            ...
            <div class="form-row">
              ...
              <div class="form-item">
                <label>出库去向</label>
                <el-select v-model="addForm.outboundDestination" placeholder="请选择" class="drawer-select">
                  <el-option label="自行利用" value="self_use"></el-option>
                  <el-option label="委外处置" value="external"></el-option>
                </el-select>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
        ...
        if (!keyword) {
          ...
          this.tableData = this.originalTableData.filter(item => {
            return (
              ...
              item.wasteName.includes(keyword) ||
              item.wasteCode.includes(keyword) ||
    ...
    handleDetail(row) { this.$message.success(`查看详情 ID: ${row.id}`); },
    ...
    handleDrawerSubmit() {
      ...
      this.$message.success(`成功添加出库单，已新增 ${this.selectedRows.length} 条记录`);
